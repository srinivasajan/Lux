...
export class KeywordExtractor {
  ...
  static normalize(text: string): string {
    return text.toLowerCase()
      // Replace typical punctuation with spaces, but keep common tech symbols (like C++, C#, .NET, Node.js)
      .replace(/[^\w\s+#.-]/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  }
   ...
   */
  static extractUniqueWords(text: string): Set<string> {
    ...
    for (const word of words) {
      if (word.length > 1 && !STOP_WORDS.has(word)) {
        // Strip trailing/leading punctuation that might have survived (e.g. at end of sentence)

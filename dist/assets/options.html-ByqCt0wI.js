import"./modulepreload-polyfill-B5Qt9EMX.js";import{P as p,Z as w}from"./profile.service-BD-M7A1g.js";const E={dailyApplicationLimit:100};class S{static async getSettings(){return typeof chrome>"u"||!chrome.storage||!chrome.storage.local?{...E}:new Promise(d=>{chrome.storage.local.get(["settings"],l=>{const r=l.settings;d({...E,...r||{}})})})}static async updateSettings(d){if(typeof chrome>"u"||!chrome.storage||!chrome.storage.local)return;const r={...await this.getSettings(),...d};return new Promise(o=>{chrome.storage.local.set({settings:r},()=>o())})}}async function $(){var g,b,f;const s=document.getElementById("profile-form"),d=document.getElementById("btn-delete"),l=document.getElementById("alert-box"),r=document.getElementById("education-container"),o=document.getElementById("experience-container"),m=document.getElementById("projects-container");function c(e,t){l.textContent=e,l.className=`alert alert-${t}`,setTimeout(()=>{l.className="alert hidden"},5e3)}function u(e){var a;const t=document.createElement("div");t.className="dynamic-entry",t.innerHTML=`
...
`)}</textarea>
    ...
    `,(a=t.querySelector(".btn-remove"))==null||a.addEventListener("click",()=>t.remove()),m.appendChild(t)}(g=document.getElementById("btn-add-education"))==null||g.addEventListener("click",()=>u()),(b=document.getElementById("btn-add-experience"))==null||b.addEventListener("click",()=>v()),(f=document.getElementById("btn-add-project"))==null||f.addEventListener("click",()=>y());async function k(){try{const e=await p.getProfile();e&&(document.getElementById("name").value=e.personal.name,document.getElementById("email").value=e.personal.email,document.getElementById("phone").value=e.personal.phone||"",document.getElementById("linkedin").value=e.personal.linkedin||"",document.getElementById("github").value=e.personal.github||"",document.getElementById("portfolio").value=e.personal.portfolio||"",document.getElementById("skills").value=e.skills.join(", "),e.education.forEach(a=>u(a)),e.experience.forEach(a=>v(a)),e.projects.forEach(a=>y(a))),r.children.length===0&&u(),o.children.length===0&&v(),m.children.length===0&&y();const t=await S.getSettings();document.getElementById("nvidiaApiKey").value=t.nvidiaApiKey||"",document.getElementById("dailyLimit").value=t.dailyApplicationLimit.toString()}catch(e){console.error(e),c("Failed to load profile data.","error")}}s.addEventListener("submit",async e=>{var h;e.preventDefault();const t=new FormData(s),a=t.get("skills"),q=a?a.split(",").map(n=>n.trim()).filter(n=>n):[],L=Array.from(r.querySelectorAll(".dynamic-entry")).map(n=>({degree:n.querySelector('[name="edu-degree"]').value,university:n.querySelector('[name="edu-university"]').value,cgpa:n.querySelector('[name="edu-cgpa"]').value,year:n.querySelector('[name="edu-year"]').value})),B=Array.from(o.querySelectorAll(".dynamic-entry")).map(n=>({company:n.querySelector('[name="exp-company"]').value,role:n.querySelector('[name="exp-role"]').value,duration:n.querySelector('[name="exp-duration"]').value,bullets:n.querySelector('[name="exp-bullets"]').value.split(`
`).map(i=>i.trim()).filter(i=>i)})),I=Array.from(m.querySelectorAll(".dynamic-entry")).map(n=>({title:n.querySelector('[name="proj-title"]').value,description:n.querySelector('[name="proj-description"]').value,stack:n.querySelector('[name="proj-stack"]').value.split(",").map(i=>i.trim()).filter(i=>i),link:n.querySelector('[name="proj-link"]').value})),A={personal:{name:t.get("name"),email:t.get("email"),phone:t.get("phone"),linkedin:t.get("linkedin"),github:t.get("github"),portfolio:t.get("portfolio")},skills:q,education:L,experience:B,projects:I};try{await p.saveProfile(A),await S.updateSettings({nvidiaApiKey:t.get("nvidiaApiKey"),dailyApplicationLimit:parseInt(t.get("dailyLimit"),10)}),c("Profile saved successfully!","success")}catch(n){n instanceof w?c(`Validation Error: ${(h=n.issues[0])==null?void 0:h.message}`,"error"):(c("An unexpected error occurred.","error"),console.error(n))}}),d.addEventListener("click",async()=>{if(confirm("Are you sure you want to delete your profile? This cannot be undone."))try{await p.deleteProfile(),s.reset(),r.innerHTML="",o.innerHTML="",m.innerHTML="",u(),v(),y(),c("Profile deleted.","success")}catch(e){console.error(e),c("Failed to delete profile.","error")}}),await k()}document.addEventListener("DOMContentLoaded",$);
